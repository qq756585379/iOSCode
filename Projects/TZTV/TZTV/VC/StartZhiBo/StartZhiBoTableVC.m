...
@property (weak, nonatomic) IBOutlet OTSBlockImageView  *upLoadIV;
...
- (void)viewDidLoad {
    ...
    [self.upLoadIV setBlock:^(OTSBlockImageView *sender) {
        [self uploadImage];
    }];
...
- (IBAction)startLive:(UIButton *)sender {
    ...
    NSString *location=[NSString stringWithFormat:@"%@,%@",[YJTOOL objectForKey:LongitudeKey],[YJTOOL objectForKey:LatitudeKey]];
    NSString *url=[NSString stringWithFormat:startLiveURL,[[AccountTool account] user_id],self.huodongTF.text,self.guangchangTF.text,@"1",self.imgUrl,[YJTOOL objectForKey:CurrentCityKey],location];
    YJLog(@"%@",url);
